...
- 用户首条消息保持自然表达，不需要先写“会话标题：...”；Agent 必须在首条回复中给出 `会话标题：⭐⭐⭐短任务名` 并写入任务目录 `prompt.md` 顶部。
- 星标含义：`⭐` 临时轻量任务；`⭐⭐` 普通任务；`⭐⭐⭐` 重要项目任务；`⭐⭐⭐⭐` 工作区或系统级任务；`⭐⭐⭐⭐⭐` 长期主线或核心生产流。
- 如果用户首条消息写明 `会话标题：...`，优先采用该标题；如果环境支持自动重命名则应用，不支持时提示用户手动改。
- 输入和参考资料放入 `01_assets/`。
...
- 如果仓库包含 `scripts/build_index.py`，登记 `TASKS.md` 后运行它更新 `INDEX.md`；不要手工维护 `INDEX.md`。
