...
- For long-lived workspaces, a root `INDEX.md` gives humans a quick overview: topic, current task paths, status, and key entry points.
- `INDEX.md` is generated by `python scripts/build_index.py`; do not maintain it by hand. After updating `TASKS.md`, run the script so the index stays fresh.
...
- On Codex Desktop, format local paths as `[label](<E:/absolute/path/file>)`; do not paste bare `E:\...` paths or use `file://`.
